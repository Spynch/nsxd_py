...
- Персистентность: журнал команд (WAL) и состояние сохраняются на диск, узлы восстанавливаются после перезапуска.
...
- WAL используется для восстановления лога после падения.
